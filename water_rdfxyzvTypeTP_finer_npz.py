...
def train_main(): 
    data_dir = "/pscratch/sd/c/chunhui/RDF_rerun/water_dataset/2000frames"
    config = {
        "batch_size": 256,
        "dropout": 0,
        "l1": 64,
    ...
    name = f'water_xyzvType_addTP{config["addTP"]}_batch{config["isBN"]}II' # II: corrected one (correct val test sequence)
    res_dir = '/pscratch/sd/c/chunhui/RDF_rerun/res/water/2000frames'
    
    model = PointNetOneXY(config["l1"], config["l2"], config["l3"], config["l4"], config["l5"], 
                            config["dropout"], config["isBN"], config["isLN"], config["addTP"], config["pooling"], "water")
